...
*/
        ...
        for(i=0; i<(int)NotEmpty.size(); i++)
        {
            ...
            TempVector_01 = PointCloud[SkeletonNode[NotEmpty[i]].InfluenceList[0]].PointPos - SkeletonNode[NotEmpty[i]].SkeletonNodePos;
            for(j=1; j<InSize; j++)
            {
                TempVector_02= PointCloud[SkeletonNode[NotEmpty[i]].InfluenceList[j]].PointPos - SkeletonNode[NotEmpty[i]].SkeletonNodePos;
...
void SpaceColonization::writeSkeleton()
{
    ...
    {
        outFile<<SkeletonNode[i].SkeletonNodePos.x<<" "<<SkeletonNode[i].SkeletonNodePos.y<<" "<<SkeletonNode[i].SkeletonNodePos.z<<" "<<SkeletonNode[i].Pid<<" "<<SkeletonNode[i].cnum<<endl;
...
void SpaceColonization::drawPointList()
{
    ...
    glBegin(GL_POINTS);
    glVertex3f(PointCloud[RootPointId].PointPos.x,PointCloud[RootPointId].PointPos.y,PointCloud[RootPointId].PointPos.z);
    glEnd();
...
void SpaceColonization::drawSkeletonList()
{
   ...
   glColor3f(0.24,0.49,0.95);
    ...
    glBegin(GL_POINTS);
    for(i=0; i<SkeletonNodeNumber; i++)
        glVertex3f(SkeletonNode[i].SkeletonNodePos.x,SkeletonNode[i].SkeletonNodePos.y,SkeletonNode[i].SkeletonNodePos.z);
    glEnd();
...
void SpaceColonization::drawProcess()
{
    ...
    for(i=0; i<PointNumber; i++)
        if(PointCloud[i].flag==false)
            outFile<<PointCloud[i].PointPos.x<<" "<<PointCloud[i].PointPos.y<<" "<<PointCloud[i].PointPos.z<<endl;
